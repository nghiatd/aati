<?php
class Default_Model_About extends Zend_Db_Table_Abstract{
    ...
    public function getAboutByTypeAbout($ID){
        $db = Zend_Registry::get('connectDB');
        $spParams = array($ID);
        try{
			$stmt = $db->query("CALL select_about_by_typeabout(?)", $spParams); 
			$results=$stmt->fetchAll(); 		  
			$datas = array();
			foreach($results as $row){
				$data['id'] = $row['id'];
    ...
    public function insertAbout($arr){
    	$db = Zend_Registry::get('connectDB');
		$spParams = array($arr['name'],$arr['link'],$arr['image'],$arr['address'],$arr['email'],$arr['introductions'],$arr['date'],$arr['typeid'],$arr['order'],$arr['status']);
		try{
    ...
    public function updateAbout($arr){
		...
		try{
			...
			$stmt->bindParam(8,$arr['date']);
			$stmt->bindParam(9,$arr['typeid']);
			$stmt->bindParam(10,$arr['order']);
			$stmt->bindParam(11,$arr['status']);
			$stmt->execute();
			$stmt->closeCursor();  
