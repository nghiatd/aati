<?php
class Default_Model_Membership extends Zend_Db_Table_Abstract{
	...
	public function getMembershipByTypeMembership($ID){
		$db = Zend_Registry::get('connectDB');
		$spParams = array($ID);
        try{
			$stmt = $db->query("CALL select_membership_by_typemembership(?)",$spParams); 
			$results=$stmt->fetchAll(); 		  
			$datas = array();
			foreach($results as $row){
				$data['id'] = $row['id'];
