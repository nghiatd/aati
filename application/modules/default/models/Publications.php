<?php
class Default_Model_Publications extends Zend_Db_Table_Abstract{
    ...
    public function getAllPublicationsActive(){
        $db = Zend_Registry::get('connectDB');
        try{
			$stmt = $db->query("CALL select_allactive_publications()"); 
			$results=$stmt->fetchAll(); 		  
			$datas = array();
			foreach($results as $row){
				$data['id'] = $row['id'];
    ...
    public function getAllPublicationsActiveByID($ID){
        $db = Zend_Registry::get('connectDB');
        $spParams = array($ID);
        try{
			$stmt = $db->query("CALL select_allactive_publications_byID(?)",$spParams); 
			$results=$stmt->fetchAll(); 		  
			$datas = array();
			foreach($results as $row){
				$data['id'] = $row['id'];
