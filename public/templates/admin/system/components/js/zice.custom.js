var documentRoot = jQuery("#documentRoot").val();
...
/*	$('.main_menu li ul li a').live({
	...
	// onload
	$('#finder').elfinder({
		url : 'http://'+ baseUrl +'/public/templates/admin/system/components/elfinder/connectors/php/connector-fileimport.php',
	...
	$('.fileDialog').live('focus',function(){
		  ...
		  var type=$(this).attr('original-title');
		  fileDialogCallback(callback,type,input);										
	})
	...
	if(mybrowser.indexOf('Safari')>0){}		
	  ...
	  function fileDialogCallback(callback,type,input){
			$('<div id="finder_'+callback+'"/>').elfinder({
				 url : 'http://'+baseUrl+'/public/templates/admin/system/components/elfinder/connectors/php/connector-'+type+'.php', editorCallback : function(url) { $('#'+input).val(url);
				},closeOnEditorCallback : true, dialog : { title : 'File manager',modal : true,width : 700 }
			})							   
	  }
	  function fileDialog(callback,type){
			$('<div id="finder_'+callback+'"/>').elfinder({
				  url : 'http://'+baseUrl+'/public/templates/admin/system/components/elfinder/connectors/php/connector-'+type+'.php',dialog : { title : 'File manager',modal : true,width : 700 }
			})							   
	  }
